...
const heroesService = 'http://localhost:3333';// this link change when the heros service domain change
  ...
  request.post({
    ...
    if (!err) {
      ...
      try {
        ...
        res.status(400).send({ problem: `Verify the threat, it's seems dosn't exist`});
      }
    } else {
